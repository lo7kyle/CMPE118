...
uint8_t InitSubHSMGoBeacon1(void) {
    ES_Event returnEvent;

    t_forwardCenter = t_forward2;
    CurrentState = InitPSubState;
    returnEvent = RunSubHSMGoBeacon1(INIT_EVENT);
...
ES_Event RunSubHSMGoBeacon1(ES_Event ThisEvent) {
    ...
    static uint8_t tapeOrWall, lightOrBump = 0;
            ...
            if (ThisEvent.EventType != ES_NO_EVENT) { // An event is still active
                switch (ThisEvent.EventType) {
                    case ES_ENTRY:
                        Roach_LeftMtrSpeed(v_L_turn90ccw);
                        Roach_RightMtrSpeed(v_R_turn90ccw);
                        ES_Timer_InitTimer(FANCY_ROACH_TIMER, t_turn90 - 150);
                        break;
                    case ES_TIMEOUT:
            ...
            if (ThisEvent.EventType != ES_NO_EVENT) {
                switch (ThisEvent.EventType) {
                    case ES_ENTRY:
                        ...
                        ES_Timer_InitTimer(FANCY_ROACH_TIMER, t_turn30Center - 150);
                        break;
                    case ES_EXIT:
                        Roach_LeftMtrSpeed(0);
                        Roach_RightMtrSpeed(0);
                    ...
                    case ES_TIMEOUT:
                        if (prevState == FIX_POSITION) {
                            ...
                            nextState = GoForward1;
                            makeTransition = TRUE;
                        } else if (prevState == TURN_30_LEFT_CENTER) {
                            nextState = GoForward3;
                            makeTransition = TRUE;
                        }
                        break;
            ...
            if (ThisEvent.EventType != ES_NO_EVENT) { // An event is still active
                switch (ThisEvent.EventType) {
                    case ES_ENTRY:
                        Roach_LeftMtrSpeed(v_L_backUp);
                        Roach_RightMtrSpeed(v_R_backUp);
                        ES_Timer_InitTimer(FANCY_ROACH_TIMER, t_backUp-150);
                        break;
                    case ES_TIMEOUT:
                        nextState = TURN90CCW;
                        makeTransition = TRUE;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunSubHSMGoBeacon1(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunSubHSMGoBeacon1(ENTRY_EVENT); // <- rename to your own Run function
    }
