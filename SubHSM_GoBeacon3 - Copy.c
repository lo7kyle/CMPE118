...
ES_Event RunSubHSMGoBeacon3(ES_Event ThisEvent)
{
    ...
    static uint32_t now, time = 0;
    static uint32_t tapeOrWall= 0;
            ...
            if (ThisEvent.EventType != ES_NO_EVENT) { // An event is still active
                switch (ThisEvent.EventType) {
                    case ES_ENTRY:
                        ...
                        ES_Timer_InitTimer(FANCY_ROACH_TIMER, t_forwardCenter);
                        now = ES_Timer_GetTime();
                        break;
                     case BUMPED:
                        ES_Timer_StopTimer(FANCY_ROACH_TIMER);
                        t_forwardCenter += 500;
                        t_forwardCenter = t_forwardCenter - (ES_Timer_GetTime() - now);
                        printf("bump: %d\n",t_forwardCenter);
                        if (ThisEvent.EventParam == 0x1) {
